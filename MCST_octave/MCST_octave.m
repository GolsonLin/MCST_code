...
%     Screen('Preference', 'SkipSyncTests', 0);
    ...
    black = BlackIndex(screenNumber);
     ...
     %for debug
##    [window, windowRect] = Screen('OpenWindow',screenNumber,[0 0 0], [0 0 1050 680]);
    % Open the screen
...
%     background(241:440,621:745,:)=para.bt1;
    ...
    KbStrokeWait;
    sca;
    header = {'No.','Rule','Atribute', 'Resp', 'RT', 'Correct'};
    
##    data_table = cell2table(data, 'VariableNames', header);
    % Create a csv file to save data
    exp_data = strcat([filename '\'], 'exp_', date, '.txt');

    fid = fopen ( exp_data, 'w+');
...
for k=1:size(data,1)
  if k<10
fprintf (fid,' %d    %d     %d%d%d       %d      ',k,data{k,1},data{k,2}(1),data{k,2}(2),data{k,2}(3),data{k,3}); 
else
  fprintf (fid,' %d   %d     %d%d%d       %d      ',k,data{k,1},data{k,2}(1),data{k,2}(2),data{k,2}(3),data{k,3}); 

end
if data{k,4}<=9.999999
fprintf (fid,'%3.6f      %d \n',data{k,4},data{k,5});
else
fprintf (fid,'%3.6f     %d \n',data{k,4},data{k,5});
end
...
result=cell2struct (data,header(2:end), 2);
save([filename '\result.mat'],'result')
disp('Succeed!');
catch
    %     ShowCursor
    Screen('CloseAll');
